...
// Re-parsing every <style> block's text on each hover is expensive on
// font-heavy pages. Cache a family→descriptor map per document, rebuilt only
// when the stylesheet set changes.
const _descCache = new WeakMap(); // doc → { sig, map }

function descSignature(doc) {
...
function descIndex(doc) {
  const sig = descSignature(doc);
  const hit = _descCache.get(doc);
  if (hit && hit.sig === sig) return hit.map;
  const map = new Map();
  for (const desc of fontFaceBlocks(doc)) {
    const fam = unquote(desc['font-family'] || '').toLowerCase();
    if (fam && !map.has(fam)) map.set(fam, desc);
  }
  _descCache.set(doc, { sig, map });
  return map;
}

function findFontFaceDesc(family, doc) {
  return descIndex(doc).get(family.toLowerCase()) || null;
}
